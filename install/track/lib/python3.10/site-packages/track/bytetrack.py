...
class PeopleTrackNode(Node):
    def __init__(self, args):
        ...
        self.center_people_pixel=[]
        self.depth_value = 0  
        self.has_detection = False
        self.pub_track_data = None  # 需要在 ROS 中正确初始化
        # ------------ ROS 相關 -------------
    ...
    #--------tracking----------
            ...
            if self.locked_target is None:
                ...
                msg.data = [int(self.center_x)-640]
            ...
            for target in online_targets:
                ...
                if self.locked_target is None:
                    ...
                    if self.locked_target.track_id == track_id:
                        ...
                        self.center_people_pixel.append((self.x1+self.x2)/2)
                        self.center_people_pixel.append((self.y1+self.y2)/2)
                        self.people_center_msg = Float32MultiArray()
                        self.people_center_msg.data = self.center_people_pixel
                        if use_realsense:
                            # 建立 ROS msg
                            msg = Int32MultiArray()
                            # 假設只想要傳此刻中心點 (cx, cy)
