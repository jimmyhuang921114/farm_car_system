...
class PeopleTrackNode(Node):
    ...
    def get_center_depth(self, depth_image, x1, y1, x2, y2):
        ...
        if (cx < border_margin or cy < border_margin 
            or cx >= depth_image.shape[1]-border_margin 
            or cy >= depth_image.shape[0]-border_margin):
            return 0

        # 获取深度值时使用区域均值代替单点
        roi_size = 5
        half_roi = roi_size // 2
        depth_roi = depth_image[max(0, cy-half_roi):cy+half_roi+1, 
                            max(0, cx-half_roi):cx+half_roi+1]
        
        # 过滤无效深度值
        valid_depths = depth_roi[(depth_roi > 300) & (depth_roi < 5000)]
